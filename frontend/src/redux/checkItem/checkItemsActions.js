...
export const getCheckItem = (queryStr, page, entryForQueryStr) => { 
  return (dispatch) => {
    ...
    let url = `http://${host}:${port}/head/headCheckList?${queryStr}&page=${page}`;
    axios
      .get(url)
      .then((result) => {
        ...
        axios
          .get(
            `http://${host}:${port}/dailyStatus/find/${result.data.headCheckList[0]._id}/entryFor/${entryForQueryStr}`
          )
          .then((result2) => {
          
            result.data.headCheckList[0].dailyStatus =
              result2.data.dailyStatus.result;
              result.data.headCheckList[0].value =
              result2.data.dailyStatus.value;
